...
<script>
    $(document).ready(function(){
        $('#table-galery').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("app.galeries.load-data") }}',
                cache:false,
            },
            columns: [
                ...
                { data: 'id', name: 'id', className:'text-center', render:function(data,row,value){
                    return '<div class="btn-group">'+
                                '<a href="/app/galery/'+value.id+'/edit" class="btn btn-sm btn-warning edit-data"><i class="fe-edit"></i> Edit</a>'+
                                '<button type="button" class="btn btn-sm btn-danger hapus-data" data-id="'+value.id+'"><i class="mdi mdi-trash-can"></i> Hapus</button>'+
                            '</div>';
                } },
            ],          
            search: {
        ...
        $("#table-galery").on('click','.hapus-data[data-id]',function(e){
            e.preventDefault();
            var id = $(this).data("id");
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ route('app.galeries.destroy') }}",
                        type: 'DELETE',
                        dataType: "JSON",
                        headers: {
